...
class BlackJackApp:
    ...
    def setup_ui(self):
        ...
        self.money_label.grid(row=0, column=0, padx=10, sticky="ew")

        self.bet_label = ttk.Label(self.top_frame, text="💵 Bet: $0", style="Large.TLabel")
        self.bet_label.grid(row=0, column=1, padx=10, sticky="ew")

        # Center the labels in the top frame
    ...
    def update_hand_labels(self, show_dealer_first_card=False):
        ...
        self.draw_hand(self.player_hand, 100, 300, "Player's Hand")
        if show_dealer_first_card:
            self.draw_hand([self.dealer_hand[0]], 100, 100, "Dealer's Hand")
        else:
            self.draw_hand(self.dealer_hand, 100, 100, "Dealer's Hand")
    ...
    def draw_hand(self, hand, x, y, label):
        ...
        start_x = x + (self.canvas.winfo_width() - total_width) // 2
